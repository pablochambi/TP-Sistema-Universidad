...
public class testUniversidad {
	...
	public void queSePuedaCrearUnCurso() {
		...
		String nomMat = "Programacion basica 2";
		
		Integer numeroIns = 100;
		...
		Dias dias = Dias.LUNES_Y_MIERCOLES;
		Hora hora = Hora._08a12;
		
		Materia materia = new Materia(codMat, nomMat);
		CicloLectivo cicloL = new CicloLectivo(numeroIns, fechaInscripcion, 
	...
	@Test
	public void queNoSePuedaAsignarUnProfesorA2CursosConElMismoHorarioYCicloLectivo() {
		
		Universidad unlam = new Universidad("Unlam");
		
		Integer codCurso = 1, codOtroCurso=2, codMat = 22, codOtraMat = 29, anio = 2024,
				codAula = 333, codAula2=20,cantMax = 5, dniPro = 4,dniPro2=5;
		String nomMat = "Programacion basica 2",nomPro="Juan", apePro = "Montan",
				nomPro2="Diego", apePro2 = "Maradona",fechaInicio = "02/03/2023";
		TipoCuatri tipoCuatri = TipoCuatri._1ER_C;
		Dias dias = Dias.LUNES;
		...
		Materia materia = new Materia(codMat, nomMat);
		Materia otraMateria = new Materia(codOtraMat, "Base de Datos 1");
		...
		Profesor profe = new Profesor(dniPro, nomPro, apePro);                                      //Unico Profe
		
		assertTrue(unlam.registrarMateria(materia));
		assertTrue(unlam.registrarMateria(otraMateria));
		assertTrue(unlam.registrarAula(aula));
		assertTrue(unlam.registrarAula(aula2));
		
		assertTrue(unlam.crearUnCurso(codCurso,codMat,horario,cicloLectivo,codAula));
		assertTrue(unlam.crearUnCurso(codOtroCurso,codOtraMat,horario,cicloLectivo,codAula2));//Otro curso pero con igual horario y ciclo lectivo que el anterior curso
		
		assertTrue(unlam.ingresarProfesor(profe));
		assertTrue(unlam.inscribirUnProfeAUnCurso(dniPro, codCurso));
		assertFalse(unlam.inscribirUnProfeAUnCurso(dniPro, codOtroCurso));//Esto debe dar falso
		
		assertEquals((Integer)1, unlam.cantDeProfesDeUnCurso(codCurso));
		assertEquals((Integer)0, unlam.cantDeProfesDeUnCurso(codOtroCurso));
		
	}
	
	
	@Test
	public void queNoSePuedaInscribirMasAlumnosQueLaCapacidadDelAula() {
		
		Universidad unlam = new Universidad("Unlam");
		
		Integer codCurso = 1, codMat = 22, anio = 2024,
				codAula = 333, cantMax = 4, dniPro = 4,dniPro2=5,
				dniAlu = 12	,dniAlu2=14;
		String nomMat = "Programacion basica 2",nomPro="Juan", apePro = "Montan",
				nomPro2="Diego", apePro2 = "Maradona",fechaInicio = "02/03/2023",
				nomAlu = "Carlos", apeAlu ="Rodriguez",
		...
		assertTrue(unlam.ingresarAlumno(alumno1));
		assertTrue(unlam.ingresarAlumno(alumno2));
		assertTrue(unlam.ingresarAlumno(new Alumno(3, nomAlu2, apeAlu2)));
		assertTrue(unlam.ingresarAlumno(new Alumno(4, nomAlu2, apeAlu2)));
		assertTrue(unlam.ingresarAlumno(new Alumno(5, nomAlu2, apeAlu2)));
		...
		assertTrue(unlam.inscribirUnAlumnoAUnCurso(dniAlu, codCurso));
		assertTrue(unlam.inscribirUnAlumnoAUnCurso(dniAlu2, codCurso));
		assertTrue(unlam.inscribirUnAlumnoAUnCurso(3, codCurso));
		assertTrue(unlam.inscribirUnAlumnoAUnCurso(4, codCurso));
		assertFalse(unlam.inscribirUnAlumnoAUnCurso(5, codCurso));//NoSePuedeInscribir
		
		Integer cantDeAlumnosEnUnCursoEsperado = 4;
		Integer cantDeAlumnosEnUnCurso = unlam.cantDeAlumnosDeUnCurso(codCurso);
		assertEquals(cantDeAlumnosEnUnCursoEsperado, cantDeAlumnosEnUnCurso);
	}
	
	//queNoSePuedaCrearDosCursosConLaMismaMateriaHorarioCicloLectivoYCodAula
